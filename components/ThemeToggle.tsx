...
const ThemeToggle = () => {
  ...
  return (
    <div className="fixed top-2 right-4">
      <button
        onClick={toggleTheme}
        className="bg-primary/10 relative flex h-8 w-14 items-center rounded-xl border px-1 backdrop-blur-lg transition-colors duration-30"
        aria-label="Toggle Theme"
      >
        <div
          className={`pointer-events-none absolute inset-0 z-40 flex items-center justify-between px-2 text-xs text-white`}
        >
